...
console.log(basketArr);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, field: keyof Order) => {
  ...
  const  handleCheckout = async () => {
    ...
    if(loginState?.username && loginState.username.length !== 0 ){
      if(newdata.delivery_address.length!=0&&newdata.contact.length!=0&&newdata.payment_method.length!=0){
        try {
    ...
    <Box className='flex' gap='19px' flexDirection='row-reverse'>
      {!showCheckoutOr ? (
        <>
          ...
          <Box className='flex flex-col mt-4 bg-white40' width='618px' height='515px'>
            <Box className='ml-10 mt-10'>
              <Text marginBottom='23px' fontSize='3xl' className='text-color-#4F4F4F'>
              ...
              <Box>
                ...
                <Box>
                  ...
                  <Box display='flex' alignItems='center' gap='7px'>
                    ...
                    >
                      {isChecked1 && (
                        <Box width='15px' height='15px' borderRadius='full' bg='#6FCF97' />
                      )}
                    </Box>
                    <Text color='#6FCF97' marginEnd='auto' width='112px' height='21px'>
                    ...
                    <Text>{translate("the door by credit card", locale)}</Text>
                  </Box>
                  <Button
                    marginTop='32px'
                    width='542px'
