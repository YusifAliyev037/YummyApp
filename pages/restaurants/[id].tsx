...
import { getProducts, getRestaurants } from '@/shared/AdminComponents/Services/axios';
import { Box, Text, Image, TableContainer, Table, Tbody, Tr, Td, Button } from '@chakra-ui/react';
...
const RestaurantPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function fetchProducts() {
      try {
        ...
        const mainProduct = resArr.filter((item: any) => item.rest_id === local?.name);
  ...
  return (
    <>
      <ClientHeader />
      <Box className='px-8'>
        <Box as='section' key={local.id}>
          <Image className='w-full h-[450px] object-cover' src={local.img_url} alt={local.name} />
          <Box className='flex justify-around m-3'>
            <Box className='pr-80'>
              <Text className='font-bold text-xl text-gray600'>{local.name}</Text>
              <Text className='font-medium text-sm text-gray300'>{local.address}</Text>
            </Box>
            <Box className='flex gap-8'>
              <Box>
                <Text className='text-gray300 font-medium text-lg'>Cuisine</Text>
                <Text className='text-gray300 font-medium text-sm'>{local.cuisine}</Text>
              </Box>
              <Box className='border border-2 border-red400 rounded-md p-1 px-2'>
                <Text className='font-normal text-sm text-red400'>${local.delivery_price}</Text>
                ...
                <Table variant='simple'>
                  <Tbody className='overflow-7-scroll mb-11'>
                    {product.map((prod, index) => (
                      <Tr key={index}>
                        <Td>
                          <Image src={prod.img_url} className='w-14' />
                        </Td>
                        <Td>
                          <p>{prod.name}</p>
                        ...
                        <Td isNumeric>
                          <span className='text-userText'>From</span> ${prod.price}
                        </Td>
                        <Td>
                          <button
